...
class CBR(chainer.Chain):
    def __init__(self, ch0, ch1, k, s, p,
            bn=True, activation=F.relu, dropout=False):
        self.bn = bn
...
class Encoder(chainer.Chain):
    def __init__(self, in_ch=1):
        ...
        layers['c1'] = CBR(64, 128, 3, 1, 1, 
            bn=True, activation=F.leaky_relu, dropout=False)
        layers['c2'] = CBR(128, 128, 4, 2, 1,
            bn=True, activation=F.leaky_relu, dropout=False)
        layers['c3'] = CBR(128, 256, 3, 1, 1,
            bn=True, activation=F.leaky_relu, dropout=False)
        layers['c4'] = CBR(256, 256, 3, 1, 1,
            bn=True, activation=F.leaky_relu, dropout=False)
        layers['c5'] = CBR(256, 256, 4, 2, 1,
            bn=True, activation=F.leaky_relu, dropout=False)
        layers['c6'] = CBR(256, 256, 3, 1, 1,
            bn=True, activation=F.leaky_relu, dropout=True)
        layers['c7'] = CBR(256, 512, 3, 1, 1,
            bn=True, activation=F.leaky_relu, dropout=True)
        layers['c8'] = CBR(512, 512, 3, 1, 1,
            bn=True, activation=F.leaky_relu, dropout=True)
...
class Decoder(chainer.Chain):
    def __init__(self, out_ch=1):
        layers = {}
        w = chainer.initializers.Normal(0.02)
        layers['c0'] = CBR(512, 512, 3, 1, 1,
            bn=True, activation=F.relu, dropout=True)
        layers['c1'] = CBR(512, 256, 3, 1, 1,
            bn=True, activation=F.relu, dropout=True)
        layers['c2'] = CBR(256, 256, 3, 1, 1, 
            bn=True, activation=F.relu, dropout=True)
        layers['c3'] = CBR(256, 256, 4, 2, 1,
            bn=True, activation=F.relu, dropout=False)
        layers['c4'] = CBR(256, 128, 3, 1, 1,
            bn=True, activation=F.relu, dropout=False)
        layers['c5'] = CBR(128, 128, 3, 1, 1,
            bn=True, activation=F.relu, dropout=False)
        layers['c6'] = L.DeconvolutionND(3, 128, 64, 4, 2, 1, initialW=w)
...
class PixDis(chainer.Chain):
    def __init__(self, in_ch=1, out_ch=1):
        layers = {}
        w = chainer.initializers.Normal(0.02)
        layers['c0_i'] = L.Convolution2D(in_ch, 64, 4, 2, 1, initialW=w)
        layers['c0_c'] = L.Convolution2D(in_ch, 64, 4, 2, 1, initialW=w)
